...
### Getting Started
Make sure you have node and npm installed. You can check that by running "node -v" and "npm -v" on your computer. Within the root directory, should be called "project-t02-clubrush", run "npm install". This installs all the dependencies listed within the package.json file. Also run "npm run build". This will create a build folder that contains static files for rendering frontend from the backend. Within the same root directory, create a file called ".env" to hold some variables that we will use. Within that file, add the code: ```REACT_APP_website="http://localhost:9000"``` and on another line ```REACT_APP_frontend="http://localhost:3000"```. Now we need to generate our own mongodburi. After generating the MongoDB URI, insert it into the ".env" file with the respective user and password. <br/>
...
***IMPORTANT. Make sure you only run the homepage once, so that the route /testAPI is only called once, and only uploads once to your database. Calling homepage more than once can lead to multiple copies of data being saved to your database.*** <br/>
